...
This project focuses on **data preprocessing for customer churn analysis** using Python, Pandas, NumPy, Matplotlib, Seaborn, and Scikit-learn.
...
This is used to examine the columns, data types, and number of non-null values.
...
The mean and median of `Age` are calculated:
...
These functions are used to inspect the dataset and obtain descriptive statistics.
...
Therefore, `Age` and `Tenure` are selected as the features for the scaling exercise.
...
* Understand the use of feature scaling techniques.
...
* `Age` contains 34 missing values.
* `Gender` contains 37 missing values.
...
6. Training a machine-learning model to predict customer churn.
7. Evaluating the model using accuracy, precision, recall, F1-score, and confusion matrix.
